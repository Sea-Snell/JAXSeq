...
from JaxSeq.utils import multihost_device_put, multihost_device_get, get_gradient_checkpoint_policy
...
class FlaxT5LayerCollection(nn.Module):
    ...
    def setup(self):
        if self.config.gradient_checkpointing:
            FlaxT5CheckpointBlock = remat(
                FlaxT5Block, static_argnums=(6, 7, 8, 9), 
...
class FlaxT5PreTrainedModel(FlaxStreamGenerationMixin, FlaxPreTrainedModel):
    """
    ...
    def pad_embeddings(self, params: PyTree, param_sharding: Optional[PyTree]=None, dtype: jnp.dtype=jnp.float32) -> PyTree:
        ...
        if "shared" in params and "embedding" in params["shared"]:
            if param_sharding is not None:
                params["shared"]["embedding"] = multihost_device_get(
                    params["shared"]["embedding"], 
                    param_sharding["shared"]["embedding"], 
                )
            emb = jnp.zeros((self.config.vocab_size, self.config.hidden_size), dtype=dtype)
            params["shared"]["embedding"] = emb.at[:self.config.unpadded_vocab_size, :].set(params["shared"]["embedding"])
            if param_sharding is not None:
                params["shared"]["embedding"] = multihost_device_put(
                    params["shared"]["embedding"], 
                    param_sharding["shared"]["embedding"], 
                )
        if "lm_head" in params:
            ...
            if "bias" in params["lm_head"]:
                ...
                lm_head_bias = jnp.zeros((self.config.vocab_size,))
