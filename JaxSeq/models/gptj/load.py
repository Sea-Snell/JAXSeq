...
from JaxSeq.models.gptj.model import FlaxGPTJForCausalLM
...
) -> Tuple[TrainState, FlaxGPTJForCausalLM]:
    model = FlaxGPTJForCausalLM(model_config, dtype=model_dtype, _do_init=False)
...
def load_train_state(
    ...
    if ModelLoadMode.match_load_mode(model_load_mode, ModelLoadMode.HF):
        ...
        with jax.default_device(jax.devices('cpu')[0]):
            model, params = FlaxGPTJForCausalLM.from_pretrained(model_load_path, _do_init=False, dtype=model_dtype)
        ...
        with open(model_load_path, 'r') as f:
            model_config = GPTJConfig.from_dict(json.load(f))
...
def load_params_from_config(
    ...
    force_pad_embeddings: bool=False, 
    params_dtype: Optional[Union[str, jnp.dtype]]=jnp.float32, 
) -> Tuple[PyTree, FlaxGPTJForCausalLM]:
    model = FlaxGPTJForCausalLM(model_config, dtype=model_dtype, _do_init=False)
    model.config.mesh = mesh
    # shard params
    params = freeze(shard_params_from_config(model, prng_key, params_dtype=params_dtype))
