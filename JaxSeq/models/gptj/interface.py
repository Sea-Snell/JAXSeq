...
class GPTJInferenceMask(InferenceMask):
    @classmethod
    def load_inference(
        cls, 
        params: PyTree, 
        ...
        mesh = model.config.mesh
        assert mesh is not None
        temp_inference = GPTJInference.load_inference(params, model, tokenizer, loss_fn=None, dp_shard_logits=dp_shard_logits)
        params_partition_spec = match_partition_rules(model.config.get_partition_rules(), params)
