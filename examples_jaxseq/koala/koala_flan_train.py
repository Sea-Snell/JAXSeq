...
def main(
    ...
    def process_jsonl(x: Any):
        return [(x['inputs'], 0.0), (x['targets'], 1.0), (tokenizer.eos_token, 1.0)]

    train_data = MaskIterableDataset.packed_from_str_segments_iterable(
        MapIterable(process_jsonl, FileOpenIterable(convert_path(train_data_path), 'r', pipe=jsonl_stream)), 
        tokenizer, 
