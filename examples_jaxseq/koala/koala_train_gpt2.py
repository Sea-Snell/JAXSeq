...
def main(
    ...
    train_data = MaskIterableDataset.packed_from_str_segments_iterable(
        MapIterable(lambda x: list(map(lambda y: (y[0].replace("<s>", tokenizer.bos_token).replace("</s>", tokenizer.eos_token), y[1]), x))+[(tokenizer.eos_token, 1.0)], FileOpenIterable(convert_path(train_data_path), 'r', pipe=jsonl_stream)), 
        tokenizer, 
    ...
    eval_data = MaskIterableDataset.packed_from_str_segments_iterable(
        MapIterable(lambda x: list(map(lambda y: (y[0].replace("<s>", tokenizer.bos_token).replace("</s>", tokenizer.eos_token), y[1]), x))+[(tokenizer.eos_token, 1.0)], FileOpenIterable(convert_path(eval_data_path), 'r', pipe=jsonl_stream)), 
        tokenizer, 
        max_length=max_length, 
        truncate=True, 
        buffer_start_str=tokenizer.bos_token, 
