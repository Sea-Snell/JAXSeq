...
from JaxSeq.utils import jsonl_stream, convert_path, load_mesh, get_dtype, setup_experiment_save, get_enabled_save_path
import jax
import jax.numpy as jnp
from JaxSeq.utils import BlockingStrategy, Padding, Truncation, uuid_name, jsonl_load, get_weight_decay_mask, create_path
import os
import optax
from JaxSeq.models.T5.interface import T5Train, T5Inference
from JaxSeq.models.T5.load import load_train_state, ModelLoadMode
...
def main(
    ...
    train_data = Seq2SeqDataset.from_str_list(
        list(map(lambda x: (x['in_text'], tokenizer.pad_token+x['out_text'].removeprefix(tokenizer.pad_token)), train_json_data)), 
        tokenizer, 
    ...
    eval_data = Seq2SeqDataset.from_str_list(
        list(map(lambda x: (x['in_text'], tokenizer.pad_token+x['out_text'].removeprefix(tokenizer.pad_token)), eval_json_data)), 
        tokenizer, 
        in_blocking_strategy=BlockingStrategy(
            padding=Padding.RIGHT, 
            truncation=Truncation.LEFT,
    ...
    def optim_getter(params: PyTree):
        mask = get_weight_decay_mask((
            re.escape("['layer_norm']['weight']"), 
            re.escape("['final_layer_norm']['weight']"), 
            re.escape("['relative_attention_bias']['embedding']"), 
        ))(params)
    ...
    def evaluator(inference: T5Inference):
        nonlocal eval_prng
